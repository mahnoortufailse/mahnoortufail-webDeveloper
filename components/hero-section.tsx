...
import { fadeInDown, scaleIn, staggerContainer, staggerItem, hoverScale } from "@/lib/animations"
import { MotionDiv, MotionButton, MotionH1, MotionP, StaggerWrapper } from "@/components/motion-wrapper"

interface HeroSectionProps {
  onDownloadResume: () => void
}

export const HeroSection = memo(function HeroSection({ onDownloadResume }: HeroSectionProps) {
  return (
    <motion.section
      id="home"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20">
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center">
          <StaggerWrapper className="space-y-6 sm:space-y-8 text-left order-2 lg:order-1">
            ...
            >
              Innovative Software Developer | Passionate About Problem-Solving
            </MotionP>
            ...
            <MotionP
              ...
              variants={staggerItem}
            >
              I'm a full-stack developer specializing in Next.js, React, TypeScript, and the MERN stack. I build fast,
              scalable, and user-friendly web applications from front-end interfaces to back-end APIs, with a focus on
              clean code, performance, and great user experience.
            </MotionP>

            <StaggerWrapper className="flex flex-wrap gap-2 sm:gap-3 py-4">
              {["React", "Node.js", "MongoDB", "Next.js", "Express", "JavaScript"].map((tech, index) => (
                <motion.div
                  key={tech}
                  variants={staggerItem}
          ...
          <StaggerWrapper className="flex flex-col items-center space-y-6 sm:space-y-8 order-1 lg:order-2">
            ...
            <StaggerWrapper className="grid grid-cols-2 gap-3 sm:gap-4 lg:gap-6 w-full max-w-xs sm:max-w-sm">
              {[
                { value: 2, suffix: "+", label: "Jobs", color: "text-primary" },
                { value: 3, suffix: "+", label: "Internships", color: "text-accent" },
                { value: 10, suffix: "+", label: "Technologies", color: "text-primary" },
                { value: 10, suffix: "+", label: "Projects", color: "text-accent" },
              ].map((stat, index) => (
                <MotionDiv
                  key={stat.label}
